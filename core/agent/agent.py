...
from core.prompts.message_aggregation import message_aggregation,inner_test
from core.prompts.post_process import post_process
from core.prompts.output_format import output_format_prompt
from core.prompts.reasoning import reasoning_prompt
...
class Agent(Node):
    ...
    async def _async_execute(self, input, spatial_info, temporal_info, **kwargs):
        ...
        if passed:
            return response
        prompt = self._process_inputs(input, spatial_info, temporal_info, **kwargs)
        response = await self.llm.agen(prompt)
        response = post_process(input, response, self.post_process)
        logger.debug(f"Agent {self.id} Role: {self.role.role} LLM: {self.llm.model_name}")
        logger.debug(f"system prompt:\n {prompt[0]['content']}")
        logger.debug(f"user prompt:\n {prompt[1]['content']}")
