...
from core.llm.llm_embedding import SentenceEncoder
from core.graph.graph import Graph
from core.utils.utils import get_kwargs, plot_embedding_heatmap, plot_row_similarity
from core.utils.globals import Cost, push_local_counters, pop_local_counters
from loguru import logger

# Cross-query parallelism for the per-batch Graph.run loop in Router.forward.
# Set via ROUTER_GRAPH_PARALLEL (default 16).
_GRAPH_RUN_WORKERS = int(os.environ.get('ROUTER_GRAPH_PARALLEL', '16'))
...
class Router(nn.Module):
    """Constrained-MORL router over a fixed LLM/role pool."""

    def __init__(self, in_dim:int = 384, hidden_dim:int = 256, max_agent:int = 6, temp:float=0.5, device=None, num_collabs:int=6):
        super().__init__()
        ...
        self.role_allocation = RoleAllocation(input_dim=in_dim, context_input_dim=3*hidden_dim, hidden_dim=hidden_dim, device=self.device, temp=temp)
        self.llm_router = LLMRouter(context_input_dim=4*hidden_dim, hidden_dim=hidden_dim, device=self.device, max_agent=max_agent, temp=1.0)
        self.collab_idx_embed = nn.Embedding(num_collabs, hidden_dim)
    ...
    def forward(self, queries:List[str], tasks:List[Dict[str, str]], 
                ...
                prompt_file:str='core/roles/finalnode/gsm8k.json'):
        """
        ...
        agent_num_int, agent_num_float, num_vae_loss = self.num_determiner(queries_embedding)

        # Cross-selector embedding of sampled collab idx
        collab_idx_emb = self.collab_idx_embed(selected_collabs_idx)
        role_input_ctx = torch.concat([query_context, collab_context, collab_idx_emb], dim=-1)

        # Role selection
        selected_roles_idx, role_log_probs, role_context, role_vae_loss = self.role_allocation(tasks_role_emb_list, role_input_ctx, agent_num_int)
        ...
        # Safety-aware routing: under adversarial evaluation, promote the
        # safety-reviewer role within the existing role pool.
        if os.environ.get('SAFETY_AWARE_ROUTING') == '1':
            for q_idx in range(len(tasks_role_list)):
                pr = next((r for r in tasks_role_list[q_idx] if r.get('Name') == 'PolicyReviewer'), None)
                if pr is not None:
                    n_ag = int(agent_num_int[q_idx].item())
                    selected_roles[q_idx] = [pr for _ in range(n_ag)]

        # LLM allocation
        llm_input_ctx = torch.concat([query_context, collab_context, role_context, collab_idx_emb], dim=-1)
        selected_llms_idx, llm_log_probs, llm_vae_loss = self.llm_router(llms_embedding, llm_input_ctx, agent_num_int, agent_num_float)
        ...
        # Per-query Graph.run is pure I/O (multi-round LLM API calls), runs in a
        # thread pool. Each worker installs its own ContextVar-backed cost counter
        # (push_local_counters), so accumulators don't race across queries — cost
        # is now ACCURATE per query, even with intra-query agent parallelism.
        def _run_one(query, task, llms_data, collab, roles):
            tokens = push_local_counters()
            try:
                kwargs = get_kwargs(collab['Name'], len(llms_data))
                llm_names = [llm['Name'] for llm in llms_data]
                role_names = [role['Name'] for role in roles]
                logger.info(f'[par] Q: {query[:120]}  task={task["Name"]}  '
                            f'rsn={collab["Name"]}  llms={llm_names}  roles={role_names}')
                ...
                try:
                    result = g.run(inputs={"query": query}, num_rounds=kwargs["num_rounds"])[0][0]
                except Exception as e:
                    logger.warning(f"graph.run error: {e}")
                    result = ""
                # Read this rollout's actual cost before popping the local counter.
                this_cost = Cost.instance().value
                return result, this_cost
            finally:
                pop_local_counters(tokens)

        per_query_args = list(zip(queries, selected_tasks, selected_llms, selected_collabs, selected_roles))
        n_workers = max(1, min(_GRAPH_RUN_WORKERS, len(per_query_args)))
        with ThreadPoolExecutor(max_workers=n_workers) as ex:
            results_with_cost = list(ex.map(lambda a: _run_one(*a), per_query_args))
        final_result = [r for r, _ in results_with_cost]
        costs = [c for _, c in results_with_cost]
        # Roll the per-rollout costs up into the global Cost (for any legacy reader).
        Cost.instance().value += sum(costs)
    ...
    def encoder_roles(self):
        """Load role JSONs from core/roles/<domain>/ and encode them.

        Returns (task_role_database, task_role_emb). Results are cached after
        the first call.
        """
        if getattr(self, '_cached_role_db_emb', None) is not None:
            return self._cached_role_db_emb
        ...
        for task in os.listdir(path):
            ...
            if os.path.isdir(task_path):
                ...
                roles_list = []
                for role in sorted(os.listdir(task_path)):
                    if not role.endswith('.json'):
                        continue
        ...
        self._cached_role_db_emb = (task_role_database, task_role_emb)
        return self._cached_role_db_emb
...
class LLMRouter(torch.nn.Module):
    def __init__(self, input_dim:int=384, context_input_dim:int=192, hidden_dim:int=256, temp:float=1.0, max_agent:int=6, device=None):
        super().__init__()
        self.device = device if device is not None else torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        self.llm_encoder = VAE(input_dim, hidden_dim, hidden_dim)
        self.context_encoder = nn.Linear(context_input_dim, hidden_dim)
        self.llm_context_encoder = GFusion(d_model=hidden_dim)
        self.temp = temp
        self.max_agent = max_agent

    def forward(self, llms:torch.Tensor, contexts:torch.Tensor, agent_num_int:torch.Tensor, agent_num_float:torch.Tensor):
        llm_hat, llm_z, llm_mu, llm_log_var = self.llm_encoder(llms)
        llm_embedding = F.normalize(llm_z, p=2, dim=1)
        contexts_embedding = self.context_encoder(contexts)
        contexts_embedding = F.normalize(contexts_embedding, p=2, dim=1)
        llm_embedding_attn = self.llm_context_encoder(llm_embedding, contexts_embedding)
        llm_embedding = F.normalize(llm_embedding_attn, p=2, dim=1)
        vae_loss = vae_loss_function(llm_hat, llms, llm_mu, llm_log_var)

        scores = torch.matmul(contexts_embedding, llm_embedding.T)
        scores = torch.softmax(scores/self.temp, dim=1)
        scores_cumsum = torch.cumsum(scores, dim=1)
