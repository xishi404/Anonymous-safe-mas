...
from core.utils.utils import find_mode
from core.agent.agent_registry import AgentRegistry

# Intra-query (wave) parallelism: when multiple nodes have zero in-degree in
# the same wave, run them concurrently in threads. ContextVars (per-rollout
# Cost counter etc.) are propagated via contextvars.copy_context().
# Disable by setting env GRAPH_INTRA_QUERY_PARALLEL=0.
_INTRA_PARALLEL = os.environ.get('GRAPH_INTRA_QUERY_PARALLEL', '1') == '1'
...
class Graph(ABC):
    ...
    def _execute_node(self, node_id, inputs, max_tries):
        """Run a single node with limited retries. Same try/except pattern as
        before; broken out so wave-parallel can call it via thread pool."""
        tries = 0
        while tries < max_tries:
            try:
                self.nodes[node_id].execute(inputs)  # output saved in node.outputs
                return
    ...
    def run(self, inputs: Dict[str,str],
        ...
        for round in range(num_rounds):
            ...
            while zero_in_degree_queue:
                # Wave-parallel: every node in zero_in_degree_queue is independent
                # (no remaining spatial predecessors), so run them concurrently.
                current_wave = list(zero_in_degree_queue)
                zero_in_degree_queue.clear()
                if _INTRA_PARALLEL and len(current_wave) > 1:
                    # Each inner thread needs its OWN copy of the context — a single
                    # Context object can't be entered concurrently from multiple
                    # threads (raises "cannot enter context: already entered").
                    # copy_context() snapshots all ContextVars (incl. per-rollout
                    # cost counter), and each fresh copy can run independently.
                    with ThreadPoolExecutor(max_workers=len(current_wave)) as ex:
                        def _run_in_fresh_ctx(nid):
                            return contextvars.copy_context().run(
                                self._execute_node, nid, inputs, max_tries)
                        futs = [ex.submit(_run_in_fresh_ctx, nid) for nid in current_wave]
                        for f in futs:
                            f.result()
                else:
                    for nid in current_wave:
                        self._execute_node(nid, inputs, max_tries)
                # Decrement successors' in-degrees AFTER the wave completes.
                for nid in current_wave:
